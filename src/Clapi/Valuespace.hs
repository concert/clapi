...
import Clapi.Valuespace.ErrWrap (Wraps(..), MonadErrors, throw, liftEither)
  ...
  where
    ...
    atPath p dc = do
      ...
      if exists
        ...
        else pathError p $ throwError $ PEAccessError NodeNotFound
      return dc
    ...
    applyTpChange tdef tpid (att, tdOp) = tpErrors p tpid $ case tdOp of
      OpSet t wvs i -> do
        ...
        eitherModifying vsTree $
          first (\s -> [PEErrorString s]) . Tree.setTpAt att p tpid t wvs i
        modifying vsTac $
          Vs2Xrefs.updateTpTas (Vs2Xrefs.Referer p) tpid tyAsserts
      OpRemove -> do
        eitherModifying vsTree $
          first (\s -> [PEErrorString s]) . Tree.removeTpAt att p tpid
...
updateContainer p cOps = pathError p $ do
  ...
  case def of
    ArrayDef {} -> eitherModifying vsTree
      $ first PEErrorString . Tree.applyReorderingsAt p cOps
...
revalidatePathData def@(TupleDef {tupDefILimit = ilimit}) p = case ilimit of
  ...
  where
    ...
    atNode f = \case
      Just rtn -> Just <$> f rtn
      Nothing -> pathError p $ throwError $ PEAccessError $ NodeNotFound
