...
import Clapi.Types.AssocList (AssocList, unAssocList, alFromZip, alLookup)
import Clapi.Types.Base (InterpolationLimit(..))
import Clapi.Types.Path (Seg, mkSeg, unSeg)
import Clapi.Types.Tree (TreeType)
import Clapi.Types.TypeName (TypeNamespace(TnTree, TnCreate), ChildTypeName, childTypeNameToText, childTypeNameFromText)
...
type family TypeNsFor a :: TypeNamespace
type instance TypeNsFor ClientPermission = 'TnTree
-- type instance TypeNsFor Liberty = 'TnTree
...
--   childTypeFor :: Seg -> metaType -> Maybe (ChildTypeName 'TnTree)
...
data StructDefinition a = StructDefinition
  { strDefDoc :: Text
  , strDefTypes :: AssocList Seg (ChildTypeName (TypeNsFor a), a)
  } deriving (Show, Eq)
...
--       [ WireValue d
--       , WireValue $ unSeg <$> names
--       , WireValue $ childTypeNameToText <$> tps
...
--         names <- mapM mkSeg ns
--         typeNames <- mapM childTypeNameFromText tns
...
data ArrayDefinition = ArrayDefinition
  { arrDefDoc :: Text
  , arrDefChildType :: ChildTypeName 'TnTree
  , arrDefChildPermission :: ClientPermission
  , arrDefCreateType :: Maybe (ChildTypeName 'TnCreate)
  } deriving (Show, Eq)
...
--     [ WireValue d
--     , WireValue $ childTypeNameToText ct
...
--         <$> childTypeNameFromText tn <*> safeToEnum (fromIntegral l)
...
data TreeDefinition
  = TStructDef (StructDefinition ClientPermission)
  | TArrayDef ArrayDefinition
  deriving (Show, Eq)

childTypeNameFor :: Seg -> TreeDefinition -> Maybe (ChildTypeName 'TnTree)
childTypeNameFor s td = case td of
    TStructDef sd -> fst <$> alLookup s (strDefTypes sd)
    TArrayDef ad -> Just $ arrDefChildType ad

childPermissionsFor :: Seg -> TreeDefinition -> Maybe ClientPermission
childPermissionsFor s td = case td of
    TStructDef sd -> snd <$> alLookup s (strDefTypes sd)
    TArrayDef ad -> Just $ arrDefChildPermission ad
...
-- 
-- structDef :: Text -> AssocList Seg (ChildTypeName 'TnTree, Liberty) -> Definition
-- structDef doc types = StructDef $ StructDefinition doc types
-- 
-- arrayDef :: Text -> ChildTypeName 'TnTree -> Liberty -> Definition
