...
import Clapi.Util (safeToEnum)
import Clapi.Types (WireValue(..), WireType(..), Definition, enumVal)
...
typeValid :: MonadFail m => WireType a1 -> TreeType a2 b -> m (a1 :~: a2)
...
validateValue :: MonadFail m => TreeType a b -> a -> m b
validateValue = \case
    TtTime -> return
    TtEnum _ -> enumVal
...
validate :: MonadFail m => TreeType a b -> WireValue -> m b
validate tt (WireValue wt a) = do
  Refl <- typeValid wt tt
  validateValue tt a

extractTypeAssertions :: TreeType a b -> b -> [(Tagged Definition Seg, Path)]
